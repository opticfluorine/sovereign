...
public class LuaLibraryGenerator : IIncrementalGenerator
{
    ...
    private const string ScriptableFullName = "Sovereign.EngineUtil.Attributes.Scriptable";
    ...
    private static string GetMarshallerClass(INamedTypeSymbol typeSymbol)
    {
        ...
        // For generic types whose element type is a [Scriptable] type
        // (e.g. List<T>, ReadOnlySpan<T>, Dictionary<string, T>), the
        // Marshal(...) overloads are generated by LuaMarshallerGenerator in
        // the element type's assembly, not in the generic type's assembly
        // (which is typically a system assembly like System.Collections).
        // Resolve to the element type's assembly's LuaMarshaller.
        if (typeSymbol.IsGenericType)
        {
            // If any type argument is a [Scriptable] type, the Marshal(...)
            // overloads are generated by LuaMarshallerGenerator in that
            // element type's assembly (e.g. List<ItemTemplateMatch>).
            foreach (var arg in typeSymbol.TypeArguments)
            {
                if (arg is INamedTypeSymbol namedArg && IsScriptable(namedArg))
                    return $"{namedArg.ContainingAssembly.Name}.Lua.LuaMarshaller";
            }

            // Otherwise, if any type argument is a predefined marshalable
            // type (e.g. List<ulong>, ReadOnlySpan<ulong>), use the default
            // marshaller, whose preamble generates Marshal overloads for the
            // predefined collection types.
            foreach (var arg in typeSymbol.TypeArguments)
            {
                if (arg is INamedTypeSymbol namedArg && namedArg.ContainingNamespace != null)
                {
                    var argTypeName = $"{namedArg.ContainingNamespace.Name}.{namedArg.Name}";
                    if (predefinedTypes.Contains(argTypeName))
    ...
    /// <summary>
    ///     Returns true if the given type is annotated with [Scriptable].
    /// </summary>
    private static bool IsScriptable(INamedTypeSymbol typeSymbol)
    {
        return typeSymbol.GetAttributes().Any(a =>
            a.AttributeClass != null
            && a.AttributeClass.ToDisplayString() == ScriptableFullName);
    }
